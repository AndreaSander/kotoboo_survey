...
caregivers_sums <- caregivers %>%
  ...
  mutate(percentage = round(sum / nrow(caregivers) * 100)) %>%
...
5. Internet search mode and mean
```{r}
...
6. Comics and participation mode and mean
```{r}
...
## ----- Important features rating: Which features were top/bottom rated? -----
### Descriptives
...
plot_feature_top3_bottom3 <- data_feature %>%
  ...
  ggplot(aes(x = fct_rev(factor(feature, levels = as.vector(mean_feature_rating$feature))), 
             y = prop, 
             fill = factor(rating, levels = c("1", "2", "3", "others", "12", "13", "14", "NA")))) +
  geom_bar(position="stack", stat="identity") +
  theme_minimal(base_size = 20) +
  scale_fill_manual(values = c("#a50026", "#e31a1c", "#fb9a99", "#ffffff", "#6baed6", "#3182bd", "#08519c", "#cccccc")) +
  coord_flip() +
  labs(x = "Feature",
       y = "Proportion of participants",
       fill = "Importance rating") 
...
relation_model <- data_relation %>%
  ...
  glm(binary_feature ~ how_often_internet * binary_country, 
        family = binomial, 
        data = .)
