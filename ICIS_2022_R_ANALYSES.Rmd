...
```{r}
#First we will need dummy variables
library(fastDummies)

lm1<- x_2022_may_qualtrics_data %>%
  dummy_cols(select_columns = "gender", ignore_na = T)

#Then for the DV which would be internet use, we should I guess divide it into columns and use as a binary predictor uses internet frequently or not. I think I will collapse groups as 1) Uses the internet to look for info (collapses frequently, often, ocasionally) and 0) does not use the internet (rarely, never)

ggplot(x_2022_may_qualtrics_data, x=gender, y=how_often_internet)+
  geom_boxplot()


#Maybe lm instead ? because we don;t really need the random effects right?

#how_often_internet ~ gender + country ??
```
