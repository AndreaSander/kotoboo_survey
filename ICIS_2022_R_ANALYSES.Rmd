...
x_2022_september_qualtrics_data2 <- x_2022_september_qualtrics_data %>%
...
## 4. caregivers' breakdown---- not relevant any more as we decided to only keep parents. 
```{r}
...
relation_model_noCountry <- data_relation %>%
  select(-c(country, starts_with("importance_"))) %>%
  glm(binary_feature ~ binary_how_often_internet, 
      family = binomial, 
      data = .,
      contrasts = list(binary_how_often_internet = contr.sum(2)/2)
      )
...
broom::tidy(relation_model_noCountry) %>%
  as.data.frame %>%
  rename(SE = `std.error`, 
         z = `statistic`,
         p = `p.value`) %>%
