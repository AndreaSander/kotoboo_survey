...
x_2022_september_qualtrics_data <- x_2022_september_qualtrics_data %>%
  anti_join(exclusion_completion) #149 12 more excluded
...
## 7. Internet search mode and mean
```{r}
...
plot_internet_freq <- count_internet %>%
  ...
  mutate(n_internet = nrow(x_2022_september_qualtrics_data) - count_internet$n[is.na(count_internet$how_often_internet)],
    Percentage = n/n_internet) %>%
  # remove NAs
  filter(!is.na(how_often_internet)) %>%
  # rename items
  mutate(how_often_internet = case_when(
    str_detect(how_often_internet, "Never") ~ "Never",
    str_detect(how_often_internet, "Rarely") ~ "Rarely",
    str_detect(how_often_internet, "Occasionally") ~ "Occasionally",
    str_detect(how_often_internet, "Often") ~ "Often",
    str_detect(how_often_internet, "Frequently") ~ "Frequently")) %>%
  ggplot() + 
  geom_bar(aes(x = factor(0), y = Percentage, 
               fill = factor(how_often_internet, 
                             levels = c("Frequently", "Often", "Occasionally", "Rarely", "Never"))), 
               position="stack", stat="identity") +
  coord_flip() + 
...
## 9. Important features rating: Which features were top/bottom rated? -----
### Descriptives
...
data_relation <- x_2022_september_qualtrics_data %>%
  ...
  mutate(binary_country = factor(case_when(country %in% east ~ "East",
                                 TRUE ~ "West"),
                                 levels = c("West", "East")),
         binary_feature = case_when(importance_evidence_based <= 5 ~ 1,
                                    importance_reliable <= 5 ~ 1,
                                    importance_concrete_advice <= 5 ~ 1,
                                    TRUE ~ 0)) %>%
...
## 10. comics and participation mode and mean
```{r}
...
plot_participation <- count_participation %>%
  # calculate percent
  mutate(n_participation = nrow(x_2022_september_qualtrics_data) - count_participation$n[is.na(count_participation$participate_in_science)],
    Percentage = n/n_participation) %>%
  # remove NAs
